 ...
 * (ADR-010). The DROP → CREATE TEMP → batched INSERT prologue was copy-pasted at 5 sites (SLOP-030);
 * it lives here once. `name` is an internal constant table name (`_dirty`, `_dirty_sec`, …), never
...
export function createDirtySet(db: DB, name: string, ids: Iterable<string>): void {
  ...
  db.transaction((xs: Iterable<string>) => {
