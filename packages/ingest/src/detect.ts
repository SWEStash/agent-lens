...
// version, so a bump doesn't invalidate user triage state. The per-version rationale (what each bump
// fixed/added and why) lives in docs/DETECTOR-CHANGELOG.md — keep the history there, not inline.
 ...
 */
function isAgentOwnedPath(p: string, configDirs: string[]): boolean {
...
const RULES: Rule[] = [
  ...
  // --- Data exfiltration (MITRE ATLAS AML.T0086) ---------------------------
  {
    ...
    test(ctx) {
      ...
      // A data-reading command (or a `<` stdin redirect) must be present on the source side. `<` sits
      // outside the \b group because a word boundary against a non-word char never matches it there.
      if (!/(?:\b(?:cat|tar|zip|gzip|base64|env|printenv)\b|<)/i.test(c)) return null;
      return { evidence: evidence(ctx.command!) };
    },
  },
  {
