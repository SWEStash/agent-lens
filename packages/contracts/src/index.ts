/**
 * @agent-lens/contracts — the normalized DB row shapes that cross package boundaries (mirror of the
 * DDL in @agent-lens/core's schema.ts). This package is a **pure-types leaf**: zero runtime, zero
 * `node:` imports, so the browser `web` package can share the exact shapes the server emits without
