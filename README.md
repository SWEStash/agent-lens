...
→ the repo's `agent-lens.config.json`. Runtime knobs — the server **port**/**host** — can be set per
run (`agent-lens serve --port 5599`), persisted in the config file's `"server"` block, or via env
vars, with precedence **flag > env > file > default**; other paths/retention are environment variables
(see the [environment table in USAGE.md](docs/USAGE.md#reference)). Run `agent-lens config` to print
